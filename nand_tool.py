...
from XeCrypt import fix_page_ecc, XeCryptKeyVaultDecrypt, XeCryptSmcDecrypt,\
    NAND_HEADER, SB_2BL_HEADER, SC_3BL_HEADER, SD_4BL_HEADER, SE_5BL_HEADER, BLMagic
...
def main() -> None:
    ...
    with NANDImage(p0.read_bytes(), MODE_FLASH) as ni:
        # header
        flash_hdr = NAND_HEADER.from_buffer_copy(ni.read(sizeof(NAND_HEADER)))
        # print(flash_hdr.smc_config_offset)
        ...
        ni.seek(flash_hdr.kv_offset)
        kv_data = ni.read(flash_hdr.kv_length)
        kv_data = XeCryptKeyVaultDecrypt(bytes.fromhex("BADA5535BADA5535BADA550007BCE3DF"), kv_data)
        print(kv_data.hex().upper())

        ni.seek(flash_hdr.smc_offset)
        smc_data = ni.read(flash_hdr.smc_length)
        smc_data = XeCryptSmcDecrypt(smc_data)
        print(smc_data.hex().upper())

        ni.seek(flash_hdr.cb_offset)
        sb_hdr = SB_2BL_HEADER.from_buffer_copy(ni.read(sizeof(SB_2BL_HEADER)))
        print(bytes(sb_hdr.header.magic))

        ni.seek(flash_hdr.cb_offset + sb_hdr.header.size)
        sc_hdr = SC_3BL_HEADER.from_buffer_copy(ni.read(sizeof(SC_3BL_HEADER)))
        print(bytes(sc_hdr.header.magic))

        ni.seek(flash_hdr.cb_offset + sb_hdr.header.size + sc_hdr.header.size)
        sd_hdr = SD_4BL_HEADER.from_buffer_copy(ni.read(sizeof(SD_4BL_HEADER)))
        print(bytes(sd_hdr.header.magic))

        ni.seek(flash_hdr.cb_offset + sb_hdr.header.size + sc_hdr.header.size + sd_hdr.header.size)
        se_hdr = SE_5BL_HEADER.from_buffer_copy(ni.read(sizeof(SE_5BL_HEADER)))
        print(bytes(se_hdr.header.magic))
